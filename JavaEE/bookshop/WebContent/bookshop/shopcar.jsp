...
<%@ page session="true"%>
<%@ page import="bean.*"%>
<%@ page import="service.manage_book"%>
<%@ page import="service.manage_buybook"%>
<jsp:useBean id="book_list" scope="page" class="service.manage_book" />
<jsp:useBean id="classlist" scope="page"
	class="service.manage_bookclass" />
<jsp:useBean id="shopcart" scope="page" class="service.manage_shopcart" />
...
if (modi!=null && !modi.equals("")) {
	...
	if (shopcart.payout(request) ) {
		mesg = "你的购物车中的物品已提交给本店，你的订单号为 "+ (String)session.getAttribute("orderId") + "<br>请及时付款，以便我们发货!";
...
<table width="778">
	<tr>
     ...
     <!--%@include file="/bookshop/inc/left.inc"%-->
		...
		<td width="600">
			<p align="center"><br>
				<b><font color="#000000">我的购物车物品清单</font></b>
			</p> <%
if (!mesg.equals("") )
...
%>
	...
	%>
				<tr>
					...
					<form name="change" method="post" action="shopcar.jsp">
						<td align="center"><input type="text" name="amount"
							maxlength="4" size="3" value="<%= iList.getAmount() %>">
						</td>
						<td align="center" width=55><input type="hidden"
							name="bookid" value="<%= iList.getBookNo() %>"> <input
							type="submit" name="modi" value="修改" onclick="return(check());"></td>
						<form name="del" method="post" action="shopcar.jsp">
							<input type="hidden" name="bookid"
								value="<%= iList.getBookNo() %>">
							<td align=center width=55><input type="submit" name="del"
								value="删除"></td>
						</form>
				</tr>
				<% } 
				...
				<tr>
					<td colspan=7 align="right"><br>你选择的图书的总金额:<%= totalprice%>元&nbsp;&nbsp;总数量：<%= totalamount%>本&nbsp;</td>
				</tr>
			</table>
			<p></p>
				...
				<tr>
					<form name="payout" method="post" action="shopcar.jsp">
						<td align="right" valign="bottom"><a href="booklist.jsp">继续购书</a>&nbsp;&nbsp;&nbsp;
							...
							onclick="javascript:return(checklogin());">&nbsp;</td>
					</form>
					<form name="form1" method="post" action="shopcar.jsp">
						<td valign="bottom">&nbsp; <input type="submit" name="clear"
							value="清空我的购物车">
						</td>
					</form>
				</tr>
